...
//            rotate([0, 0, 0]) translate([0, -9.5, 0]) 
            translate([z_delta_x, z_delta_y, 0]) render(convexity = 5) linear(bushing_z, bearing_height);
        ...
        translate([-10 - bushing_xy[0], offset-z_delta_y, x_box_height/2]) {
            if(thru == true){
                ...
                translate([0, -7+z_delta_y, -xaxis_rod_distance/2]) rotate([-90, 180, 0]) pushfit_rod(bushing_xy[0] * 2 + 0.2, len);
                translate([0, -7+z_delta_y, xaxis_rod_distance/2]) rotate([-90, 0, 0]) pushfit_rod(bushing_xy[0] * 2 + 0.2, len);
            }
        }
    }
    //threaded rod
...
module x_end_idler(){
    difference() {
        x_end_base(len=45 + abs(z_delta_y), offset=-5+z_delta_y);
